...
// that could not do that. See newCleanPostgresStore's comment for the
// failure that motivated it.
//
// Deliberately duplicated from internal/store/postgres's copy rather than
// shared: the two live in different packages, and the only way to share
// test-only code between them would be a non-test helper package that
// ships in the module purely to serve tests. A ~40-line fixture is the
